...
class GamePanel extends JPanel {
  ...
  private int duberHealth;
  
  Map map = new Map();
  
  GamePanel(Human duber, Spawner spawner, Camera scroll, Mammal[] entities, Projectile[] projectiles, int health) {
    this.duber = duber;
      ...
      for (int i = 1; i < entities.length; i++) {
        ...
        if (projectiles[i] instanceof Projectile) {
          ...
          for (int j = 1; j < entities.length; j++) {
            if ((entities[j] instanceof Zombie) && (projectiles[i].hitbox.intersects(entities[j].hitbox))) {
              entities[j].setHealth(entities[j].getHealth() - duber.damage);
              projectiles[i] = null;
