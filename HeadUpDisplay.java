...
class HeadUpDisplay {
  ...
  private void loadSprites() {
    try {
      ...
      sprites[5] = ImageIO.read(new File("GrayGunBox.png"));
  ...
  public void draw(Graphics g, Human duber, Entity[] entities) { //make things less lengthy
    ...
    xMinimap = ((int)-scroll.getXCamera() / 15) + (maxX - 350) - xLock + (int)-scroll.getXCamera();
    yMinimap = ((int)-scroll.getYCamera() / 22) + (maxY - 300) - yLock + (int)-scroll.getYCamera();
    g.drawImage(sprites[4], xMinimap, yMinimap, maxX / 15, maxY / 22, null);
    xMinimap = (duber.getxCord() / 15) + (maxX - 350) - xLock + (int)-scroll.getXCamera();
    yMinimap = (duber.getyCord() / 22) + (maxY - 300) - yLock + (int)-scroll.getYCamera();
    g.drawImage(sprites[1], xMinimap, yMinimap, null);
    
    for (int i = 0; i < entities.length; i++) {
      if (entities[i] instanceof Zombie) {
        xMinimap = (entities[i].getxCord() / 15) + (maxX - 350) - xLock + (int)-scroll.getXCamera();
        yMinimap = (entities[i].getyCord() / 22) + (maxY - 300) - yLock + (int)-scroll.getYCamera();
        g.drawImage(sprites[2], xMinimap, yMinimap, null);
      }
    }    
