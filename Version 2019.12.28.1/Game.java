...
class Game extends JFrame {
  ...
  Game(String title) {
    ...
    gamePanel = new GamePanel(duber, spawner, scroll, hud, entities);
    this.add(new GamePanel(duber, spawner, scroll, hud, entities));
  ...
  public void nextWave(int number) {
    ...
    for (int i = 0; i < hud.getWaveNum() * number; i++) {
      ...
      if (hud.getWaveNum() < 2){
        ...
        if (randomZombie == 1){
          spawner.spawnWalker(xRange, yRange, 100, 10, 3);
