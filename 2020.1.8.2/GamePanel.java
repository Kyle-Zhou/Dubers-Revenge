...
class GamePanel extends JPanel {
  ...
  GamePanel(Human duber, Spawner spawner, Camera scroll, HeadUpDisplay hud, Entity[] entities, Weapon[] weapons, Shop shop) {
    this.duber = duber;
    ...
    keys = new Keys(duber, weapons, spawner, hud, entities);
  }
  ...
  public void paintComponent(Graphics g) {
    ...
    for(int i = 0; i < 125; i++) {
      for(int j = 0; j < 125; j++) {
        if(map.getMap()[i][j] == 1) {
          g.drawImage(tiles[0], j * 36, i * 36, this);
        } else if (map.getMap()[i][j] == 2){
          g.drawImage(tiles[1], j * 36, i * 36, this);
        } else if (map.getMap()[i][j] == 3) {
          g.drawImage(tiles[2], j * 36, i * 36, this);
        }
      }
    }
    ...
    if (duber != null) {
      ...
      for (int i = 0; i < entities.length; i++) {
        ...
        if (entities[i] instanceof Projectile) {
          ((Projectile)entities[i]).travel(duber, mouse.getXCursor(), mouse.getYCursor());
